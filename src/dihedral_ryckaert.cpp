/* ----------------------------------------------------------------------
   dihedral_style ryckaert — static Ryckaert–Bellemans dihedral.

   E = Σ Cn cos^n(φ)  with LAMMPS polymer φ convention (trans = 180°).
...
DihedralRyckaert::DihedralRyckaert(LAMMPS *lmp)
    : Dihedral(lmp),
      c0(nullptr),
      c1(nullptr),
      c2(nullptr),
...
DihedralRyckaert::~DihedralRyckaert() {
...
void DihedralRyckaert::allocate() {
  allocated = 1;
...
void DihedralRyckaert::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR, "Incorrect args for dihedral_coeff ryckaert");
  if (!allocated) allocate();

  int ilo, ihi;
  utils::bounds(FLERR, arg[0], 1, atom->ndihedraltypes, ilo, ihi, error);

  double c0_one = utils::numeric(FLERR, arg[1], false, lmp);
  double c1_one = utils::numeric(FLERR, arg[2], false, lmp);
  double c2_one = utils::numeric(FLERR, arg[3], false, lmp);
  double c3_one = utils::numeric(FLERR, arg[4], false, lmp);
  double c4_one = utils::numeric(FLERR, arg[5], false, lmp);
  double c5_one = utils::numeric(FLERR, arg[6], false, lmp);
...
void DihedralRyckaert::compute(int eflag, int vflag) {
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    if (c > 1.0 + TOLERANCE || c < (-1.0 - TOLERANCE))
      error->one(FLERR, "Dihedral ryckaert problem: i={} j={} k={} l={}", i1,
                 i2, i3, i4);
...
void DihedralRyckaert::write_restart(FILE *fp) {
...
void DihedralRyckaert::read_restart(FILE *fp) {
  allocate();
  if (comm->me == 0) {
