...
# dodecane type sequence: CH3, CH2x10, CH3
...
def build_system(
    ...
    # Center the template at origin
    com = template.mean(axis=0)
    centered = template - com

    # 2x2x3 = 12 grid positions, use first n_mol
    nx, ny, nz = 2, 2, 3
    ...
    return np.array(positions)  # shape: (n_mol, n_atoms, 3)
...
def main() -> int:
    ...
    print(f"Placing {args.n_mol} copies in {BOX_SIZE:.2f} Å cubic box")

    all_coords = build_system(template, args.n_mol, BOX_SIZE, rng)
    write_lammps_data(args.output, all_coords, BOX_SIZE)
