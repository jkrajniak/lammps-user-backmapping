...
def extract_at(
    ...
    # Remap dihedrals (AT-only)
    n_dihedrals = 0
    dihedral_type_map: dict[int, int] = {}
    new_dihedral_type_id = 1
    for dih_line in data.dihedrals:
        dih_type = int(dih_line[1])
        a1, a2, a3, a4 = (int(dih_line[2]), int(dih_line[3]), int(dih_line[4]), int(dih_line[5]))
        if any(a in cg_atom_ids for a in (a1, a2, a3, a4)):
            continue
        if dih_type not in dihedral_type_map:
            dihedral_type_map[dih_type] = new_dihedral_type_id
            new_dihedral_type_id += 1
        out.dihedrals.append(
            [
                str(n_dihedrals + 1),
                str(dihedral_type_map[dih_type]),
                str(old_to_new[a1]),
                str(old_to_new[a2]),
                str(old_to_new[a3]),
                str(old_to_new[a4]),
