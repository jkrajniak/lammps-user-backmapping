...
# ///
"""Structural validation for melamine LAMMPS backmap vs JCC paper reference RDFs.

Compares C-C, C-N, and O-H radial distribution functions from a LAMMPS custom
dump (AT production frames) to GROMACS `.xvg` references in `ref/` from the
original melamine-formaldehyde publication data.

GROMACS alignment (paper `g_rdf_mpi -n index.ndx`, groups C*, N*, O*, H*):
- Element groups match atom names whose first letter is C/N/O/H (`topol_aa.top`);
  implemented via LAMMPS AT type -> element map (hybrid types 2-9).
- Default: **all atom pairs** (GROMACS `g_rdf` melt RDFs; rim135 precedent).
  Optional `--inter` restricts to intermolecular pairs only.
- C*-C* self-RDF uses GROMACS **ordered** pair counting (each atom as reference).
- C-C reports separate **bonded** (0.20-0.28 nm) and **melt-shell** (0.35-0.55 nm) peaks.

Usage:
  uv run compare_melamine_structure.py \\
    --dump dump.at_prod \\
...
ELEMENT_BY_TYPE_HYBRID: dict[int, str] = {
    2: "N",
    3: "C",
    4: "N",
    5: "C",
...
ELEMENT_BY_TYPE_EXTRACTED: dict[int, str] = {
    1: "N",
    2: "C",
    3: "N",
    4: "C",
...
AT_TYPES_HYBRID = frozenset(ELEMENT_BY_TYPE_HYBRID)
AT_TYPES_EXTRACTED = frozenset(ELEMENT_BY_TYPE_EXTRACTED)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare melamine LAMMPS RDF vs paper ref")
    parser.add_argument("--dump", type=Path, required=True)
    parser.add_argument("--final-data", type=Path, required=True)
    parser.add_argument("--ref-dir", type=Path, default=Path("ref"))
    parser.add_argument(
        "--mode",
        choices=("hybrid", "extracted"),
        default="hybrid",
        help="Atom type numbering in dump (hybrid=types 2-9 AT; extracted=1-8 after CG strip)",
    )
    parser.add_argument("--min-step", type=int, default=50000)
    parser.add_argument(
        "--inter",
        action="store_true",
    ...
    type_map = ELEMENT_BY_TYPE_HYBRID if args.mode == "hybrid" else ELEMENT_BY_TYPE_EXTRACTED
    at_types = AT_TYPES_HYBRID if args.mode == "hybrid" else AT_TYPES_EXTRACTED
    ...
    print("=== melamine structural validation vs paper reference ===\n")
    print("Element groups: C*/N*/O*/H* via LAMMPS AT types (topol_aa)")
    ...
    report_lines = [
        "melamine Tier C structural validation",
