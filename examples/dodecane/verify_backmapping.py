...
def verify_thermodynamics(log_path: Path, target_temp: float = 298.0) -> None:
    ...
    #   T_reported ≈ (N_at / N_total) x T_actual
    ...
    if "TotEng" in data:
        ...
        check(
            ratio < 0.5,
            f"Production <E> = {mean_e:.1f} ± {std_e:.1f} kcal/mol (sigma/|μ|={ratio:.2f})",
        )

    if "PotEng" in data:
        pe = np.array(data["PotEng"])
        # PE should not explode (no values > 10x the production mean)
