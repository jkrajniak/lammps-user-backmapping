...
- [x] 1.1 Create a new git worktree `fix/lambda-weighting` off
  `lammps-user-backmapping` (separate from `mpi-correctness` and
  `pete-example`)

## 2. `fix_backmap` — expose lambda_global and atom2cg

- [x] 2.1 Add `double lambda_global` member to `FixBackmap` (`fix_backmap.h`)
- [x] 2.2 Update `lambda_global` in the same place/way the per-atom ramp
  currently updates `lambda[i] += alpha` (clamped to `[0, 1]`)
- [x] 2.3 Extend `FixBackmap::extract()` to return `"atom2cg"` (dim=1,
  `int*`) and `"lambda_global"` (dim=0, `double*`), alongside the existing
  `"lambda"`
- [x] 2.4 Verify `atom2cg` is forward-communicated to ghost atoms (check the
  existing `comm_forward`/`pack_forward_comm`/`unpack_forward_comm` path
  added for MPI correctness); add ghost coverage if missing
- [x] 2.5 Add/confirm restart and `grow_arrays`/`copy_arrays` handling for
...
- [x] 3.1 Remove `compute_weight(double lambda_i, double lambda_j, bool is_cg)`
  and its doc comment entirely
- [x] 3.2 Add `compute_weight3(bool same_bead, bool is_cg, double lambda_global)`
  implementing: `is_cg → 1 − λ_global`; `same_bead → (λ_global > 0) ? 1 : 0`;
  else `→ λ_global`
- [x] 3.3 Add `same_bead(const int *atom2cg, int i1, int i2)` (pair/bond),
...
- [x] 3.4 Update the file's top-of-file usage-pattern doc comment to show
  `compute_weight3()` instead of the removed `compute_weight()`

## 4. Update all 8 call sites

- [x] 4.1 `pair_backmap.cpp`: extract `atom2cg`/`lambda_global` instead of
...
- [x] 4.2 `bond_backmap_harmonic.cpp`: same pattern for `compute()` and
  `single()`
- [x] 4.3 `bond_backmap_table.cpp`: same pattern for `compute()` and
  `single()`
- [x] 4.4 `angle_backmap_harmonic.cpp`: same pattern, `same_bead` over
  `i1,i2,i3`
- [x] 4.5 `angle_backmap_table.cpp`: same pattern, `same_bead` over
  `i1,i2,i3`, both `compute()` and the energy-only path
- [x] 4.6 `dihedral_backmap_ryckaert.cpp`: same pattern, `same_bead` over
  `i1,i2,i3,i4`
- [x] 4.7 `dihedral_backmap_table.cpp`: same pattern, `same_bead` over
  `i1,i2,i3,i4`
- [x] 4.8 `fix_backmap_pairs.cpp`: same pattern for its pairwise loop
- [x] 4.9 Grep the whole `src/` tree for any remaining `compute_weight(` /
  per-atom `lam[i` weighting reads to confirm no call site was missed

## 5. Fast unit tests (must pass before any full-scale run)

- [x] 5.1 Create `tests/unit/CMakeLists.txt`: standalone project, C++17,
  `FetchContent` for GoogleTest, one test binary linked only against
  `backmap_lambda.h` (no LAMMPS headers/library)
- [x] 5.2 Create `tests/unit/test_backmap_lambda.cpp` covering:
...
- [x] 5.3 Build and run locally: `cmake -S tests/unit -B tests/unit/build &&
...
- [x] 8.1 Write a new dated research notebook entry
...
- [x] 8.2 Update `research/STATUS.md` for the PET row and any other
  affected component status
- [x] 8.3 Update `CHANGELOG.md` under `[Unreleased]` → `Fixed` (weighting
  formula) and `Added` (unit tests / CI job)
- [x] 8.4 Update any `docs/` pages describing the lambda-weighting model to
